...
def wrap_angle(x):
    ...
    '''
    pi = np.pi
    return (x + pi/6) % (pi / 3) - pi/6

@njit
def dhamiltonian(theta, neighbors, idx, new_theta, epsilon, gamma, A, rho):
    '''
    Compute the change in Hamiltonian when one theta is changed
    Inputs:
        new_theta
        idx: index of the changed theta in indices
        epsilon, gamma, A, rho: parameters
    Outputs:
    ...
    for n in range(z_max):
        ...
        if j == -1:
            continue
        dtheta_old = np.abs(wrap_angle(old_theta - theta[j]))
        dtheta_new = np.abs(wrap_angle(new_theta - theta[j]))
        dH_int += dtheta_new * (A - np.log(max(1e-10, dtheta_new))) - dtheta_old * (A - np.log(max(1e-10, dtheta_old)))
    dH = dH_0 + (gamma / (3 * np.sqrt(rho))) * dH_int
    return dH

@njit
def energy(theta, neighbors, epsilon, gamma, A, rho):
    '''
...
@njit
def metropolis_sweep(theta, neighbors, beta, delta_theta, epsilon, gamma, A, rho):
    '''
    ...
    for _ in range(N):
        ...
        dH = dhamiltonian(theta, neighbors, idx, new_theta, epsilon, gamma, A, rho)
        attempts += 1
...
@njit
def overrelaxation_sweep(theta, neighbors, beta, epsilon, gamma, A, rho):
    '''
    ...
    for idx in range(N):
        ...
        for n in range(z_max):
            ...
            if neighbor_idx == -1:
                continue
            sum_sin += np.sin(6 * theta[neighbor_idx])
            sum_cos += np.cos(6 * theta[neighbor_idx])
        ...
        old_theta = theta[idx]
        new_theta = wrap_angle((2 * phi / 6) - old_theta)
...
class Simulation:
    ...
    def __init__(
        ...
        self.theta_flat = np.random.uniform(-np.pi/6, np.pi/6, size=self.lattice.N).astype(np.float64)
    ...
    def adapt_delta(self):
        '''
        Adapt delta_theta based on acceptance rate
        '''
    ...
    def run(self, path):
        ...
        for step in therm_range:
            if self.overrelax_interval > 0 and step % self.overrelax_interval == 0:
                overrelaxation_sweep(self.theta_flat, neighbors, self.beta, self.epsilon, self.gamma, self.A, self.rho)

            attempts, accepts = metropolis_sweep(
                self.theta_flat, neighbors, self.beta, self.delta_theta, self.epsilon, self.gamma, self.A, self.rho
            )
            ...
            if step % (self.n_therm // 10) == 0:
                E = energy(self.theta_flat, neighbors, self.epsilon, self.gamma, self.A, self.rho)/self.lattice.N
...
def plot_results(path, L, T, epsilon, rho, size=20):
    ...
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    # fig.set_title(rf"Final Configuration at $T={T:.3e}$, $L={L}$")
    fig.tight_layout()
...
def plot_eta_vs_T(base_path, L):
    ...
    plt.axhline(0.25, color='red', linestyle='--', label=r'$\eta=0.25$')
...
def plot_chi_vs_L(base_path, Ts):
    ...
    markers = ['x', '^', 'v', 'o']
    plt.figure(figsize=(8, 6))
    for T, marker in zip(Ts, markers):
        plt.errorbar(
            1/L_values,
            chi_vals[T],
            yerr=chi_errs[T],
...
def plot_chi_vs_T_all_L(base_path):
    ...
    L_values = np.array(L_values)
    sorted_indices = np.argsort(L_values)
    chi_files = [chi_files[i] for i in sorted_indices]
